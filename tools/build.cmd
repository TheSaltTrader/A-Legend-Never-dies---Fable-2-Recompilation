...
rem Configure and build fable2.
rem
...
if "%REXSDK%"=="" set "REXSDK=%PROJECT_ROOT%\..\RexBlue\win-amd64"
set "SDK_DIR=%REXSDK%"
...
rem Release drops debug info for the ~500 recompiled translation units (that is
rem a lot of PDB for code nobody reads). Any other config keeps line tables, and
rem codegen emits one source line per guest instruction, so a fault resolves to
rem the exact guest instruction under a debugger. That is the whole triage
rem method - build RelWithDebInfo when chasing a crash.
...
if not exist "%SDK_DIR%\bin\rexglue.exe" (
    echo ERROR: ReXGlue SDK not found at "%SDK_DIR%"
    echo        Set REXSDK, or put the SDK at ^<project^>\..\RexBlue\win-amd64
    exit /b 1
)
if not exist "%VCVARS%" (
...
rem it. Codegen rewrites generated\default\fable2_pch.h, and inside a single
rem ninja run the precompiled header can be built from the old copy before
rem codegen replaces it - every TU then fails with "file has been modified
rem since the precompiled header was built". Generating first makes the headers
rem final before anything compiles.
echo === Codegen ===
"%SDK_DIR%\bin\rexglue.exe" codegen "%PROJECT_ROOT%\fable2_manifest.toml" || exit /b 1
