...
// rather than assuming one - a missing script must say so, not fail silently.
fs::path ToolsDir() {
...
fs::path FindToolScript(const std::string& name) {
  std::error_code ec;
  const auto candidate = ToolsDir() / name;
...
TextureTools FindTextureTools() {
  ...
  // PATH; "python" and "python3" cover the rest. The launcher first, because
  // on a machine without Python "python" resolves to the Store's alias stub,
  // which opens a shop window rather than running anything.
...
bool PackCandidate(int w, int h, int fmt) {
  ...
  if (fmt == kFmt8 || fmt == kFmt88)                 // masks, ramps, fonts, video
    return false;
  // Not colour: DXN is a tangent-space normal map, DXT5A a single channel.
  // Replacing either with an RGBA image changes what the LIGHTING reads -
  // measured on this title as skin-toned clothing and rainbow hair.
  if (fmt == kFmtDXN || fmt == kFmtDXT5A)
    return false;
...
int FormatFromName(const std::string& n) {
  static const std::pair<const char*, int> kNames[] = {
      ...
      {"k_DXT4_5", kFmtDXT45},    {"k_DXN", kFmtDXN},      {"k_DXT5A", kFmtDXT5A}};
        ...
        [&progress](const std::string& text) {
          ...
          if (text.rfind("PROGRESS ", 0) == 0) {
            ...
            if (std::sscanf(text.c_str(), "PROGRESS %lld %lld", &done, &total) == 2 &&
                total > 0) {
              progress.files_done = uint64_t(done / 1024);
              progress.files_total = uint64_t(total / 1024);
            }
          }
        },
        &rc, &progress.cancel);
...
std::thread UpscaleTexturesAsync(const TextureTools& tools, const fs::path& texture_dir,
  ...
  return std::thread([tools, texture_dir, upscale, scale, ai, ai_strength, only_missing,
    ...
    progress.files_total = uint64_t(CountDumpedTextures(texture_dir));
    ...
    const bool started = RunHidden(
        ...
        [&progress](const std::string& text) {
          ...
          if (text.rfind("PROGRESS ", 0) == 0) {
            ...
            if (fields >= 2 && total > 0) {
              progress.files_done = uint64_t(done);
              progress.files_total = uint64_t(total);
            }
            if (fields >= 3)
