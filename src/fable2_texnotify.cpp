    ...
    // the tuning and the menu own the cvar.
    {
      ...
      if (st && st->ultrawide && aspect > 1800) {
        ...
        const int menu_gap_ms = rex::cvar::GetFlagInfo("fable2_menu_letterbox_gap_ms")
                                    ...
                                    : 150;
        // Ultrawide menu handling, remade after the user's transition
        // screenshots (Error 1/2). The visible fault was NOT a character squish
        // but a torn transition: switching the presenter to 16:9 for the pause
        // menu re-lays-out the swap chain and left stale, ghosted world in the
        // ultrawide edges, AND the game's own menu-dim (a 2D overlay) was being
        // shrunk into the centre 16:9 band by the HUD scaling, so the ultrawide
        // edges never dimmed and stayed bright. Both come from treating the
        // pause menu like the front end.
        //
        // So the presenter is put to 16:9 ONLY for the FRONT END (title / main
        // menus / loading - no world behind it, and any switch there hides
        // under a load fade). While a world camera exists (gameplay AND the
        // pause / Up menu) the presenter stays edge to edge: it never
        // re-lays-out, so it never ghosts. The HUD 2D compression is applied
        // ONLY in gameplay; the pause / Up menu is left at full width so the
        // game's menu-dim covers the whole screen and the menu art is not
        ...
        if (!frontend && fable2::PauseMenuOpen()) pause_seen = now;
        const bool pause_menu = pause_seen.time_since_epoch().count() != 0 &&
