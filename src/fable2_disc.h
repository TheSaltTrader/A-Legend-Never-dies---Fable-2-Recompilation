...
// Fable II, from the disc's own XEX execution info. Anything else is
// still installable - the setup screen just says so plainly rather than
// pretending it is the right game.
inline constexpr uint32_t kFable2TitleId = 0x4D5307F1;
...
struct DiscInfo {
  ...
  bool IsFable2() const { return title_id == kFable2TitleId; }
