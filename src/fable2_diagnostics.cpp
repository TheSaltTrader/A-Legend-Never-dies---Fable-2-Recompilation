...
// The newest fable2_*.log in `dir`. The runtime numbers them per launch, so
// the newest is this session's - and that is the one a report is about.
fs::path NewestLog(const fs::path& dir) {
  ...
  for (fs::directory_iterator it(dir, ec), end; it != end; it.increment(ec)) {
    ...
    if (name.rfind("fable2", 0) != 0 || it->path().extension() != ".log")
      continue;
...
// This session's log. A scripted launch passes --log_file and the runtime
// writes there; a plain launch leaves the cvar empty and the runtime names the
// file itself under logs/. Asking the cvar first means a report from a
// scripted run does not quietly bundle an older, unrelated log.
fs::path ThisSessionsLog(const fs::path& log_dir) {
  const std::string configured = rex::cvar::GetFlagByName("log_file");
  if (!configured.empty()) {
    std::error_code ec;
    const fs::path p(configured);
    if (fs::is_regular_file(p, ec))
      return p;
  }
  return NewestLog(log_dir);
}
...
DiagnosticsResult WriteDiagnostics(const fs::path& settings_file, const fs::path& log_dir) {
  ...
  const fs::path out_file =
      out_dir / ("fable2-diagnostics-" + TimeStamp("%Y%m%d-%H%M%S") + ".txt");
  ...
  out << "Fable II recompilation - diagnostics\n";
  out << "Written " << TimeStamp("%Y-%m-%d %H:%M:%S") << "\n";
  out << "Port version: " << FABLE2_VERSION << "\n";
  out << "Executable:   " << (rex::filesystem::GetExecutableFolder() / "fable2.exe").string()
      << "\n";
  ...
  AppendFile(out, settings_file, "fable2_settings.cfg");
  AppendFile(out, rex::filesystem::GetExecutableFolder() / "cache" / "fable2_tuning.toml",
             "cache/fable2_tuning.toml");

  const fs::path log = ThisSessionsLog(log_dir);
  if (log.empty()) {
    out << "\n===== log =====\n(no fable2*.log found in " << log_dir.string()
